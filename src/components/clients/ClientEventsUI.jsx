...
export default function ClientEventsUI({ event, Link }) {
  return (
    <li key={event._id}>
      <Link
        to={`/events/${event._id}`}
        className="block bg-white p-5 rounded-xl border border-[#F3E8FF] hover:border-[#F59E0B] shadow-sm hover:shadow-md transition-all duration-200 dark:bg-gradient-to-b dark:from-[#1a1026] dark:to-black dark:border-[#F59E0B]/10 dark:hover:border-[#F59E0B]/20 cursor-default"
      >
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-[#9B2C62] text-lg mb-1 transition-colors">
              {event.name}
            </h3>
