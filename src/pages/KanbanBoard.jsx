...
const KanbanBoard = () => {
  ...
  const getColumnsFromTasks = () => {
    return {
      todo: {
        id: "todo",
        title: "To Do",
        tasks: tasks.filter(task => task.status === "To Do").map(mapTaskToCard),
        color: "#F59E0B" // Saffron gold
      },
      inProgress: {
        id: "inProgress",
        title: "In Progress",
        tasks: tasks.filter(task => task.status === "In Progress").map(mapTaskToCard),
        color: "#FF7E33" // Pumpkin
      },
      inReview: {
        id: "inReview",
        title: "In Review",
        tasks: tasks.filter(task => task.status === "In Review").map(mapTaskToCard),
        color: "#9B2C62" // Deep mulberry
      },
      completed: {
        id: "completed",
        title: "Completed",
        tasks: tasks.filter(task => task.status === "Completed").map(mapTaskToCard),
  ...
  const onDragEnd = async (result) => {
    // ... (keep your existing drag-end logic)
  };
